#!/usr/bin/env python3
"""
脚本：处理 78_degrees_of_wisdom.txt 文件并准备 RAG 数据
将文本文件分割成适合 RAG 的文档块
"""
...
def process_78degrees_file(file_path: str) -> List[Dict[str, Any]]:
    """
    处理 78_degrees_of_wisdom.txt 文件，将其转换为 RAG 文档格式
    
    Args:
        file_path: 78_degrees_of_wisdom.txt 文件路径
    ...
    # 转换为文档格式
    documents = []
    source = "78_degrees_of_wisdom.txt"
    
    for i, section_text in enumerate(sections, 1):
        # 提取元数据（如果有卡牌名称）
        metadata = extract_card_names(section_text)
        metadata['section_number'] = i
        metadata['source_file'] = '78_degrees_of_wisdom.txt'
        
        # 创建文档
        doc = {
            'text': section_text,
            'source': source,
            'chunk_id': f"78degrees-section-{i:04d}",
            'metadata': metadata
        }
...
def save_documents_json(documents: List[Dict[str, Any]], output_path: str = None) -> str:
    ...
    if output_path is None:
        ...
        output_path = str(data_dir / "78degrees_documents.json")
...
def main():
    ...
    # 默认文件路径
    default_path = "/Users/lexuanzhang/code/tarot_agent/docs/78_degrees_of_wisdom.txt"
    ...
    logger.info("=" * 60)
    logger.info("78 Degrees of Wisdom 文件处理脚本")
    logger.info("=" * 60)
    
    try:
        # 处理文件
        documents = process_78degrees_file(file_path)
        ...
        logger.info(f"  2. 运行 cd rag/scripts && python upload_to_supabase.py --file ../data/78degrees_documents.json 上传到 Supabase")
        logger.info("=" * 60)
